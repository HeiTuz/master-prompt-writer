...
```text
다음:
1. 자동 개선 — 축 하나를 자동으로 골라 다시 컴파일
```

**2번부터는 이 산출물에서 실제로 값어치 있는 것만 에이전트가 판단해 1~3개 붙인다.** 고정 목록을 돌려쓰지 않는다 — 매번 같은 세 줄이 나오면 그게 곧 장식 메뉴다.

후보는 MPW에 실제로 있는 연산이어야 한다: 축 지정 **MODEL-델타** / 표면 재판정 후 **다른 모델·플랫폼으로 재컴파일**([surfaces.md](references/image/surfaces.md) → [model-routing.md](references/image/model-routing.md)) / 컷 분리 / 레인 전환 / 그 표면의 파라미터 조정 / 체인 다음 단계로 넘기기.
...
이모지 번호·"선택하세요"·표는 쓰지 않는다. 사용자가 이미 다음 행동을 말한 맥락이면 붙이지 않고, 자율 루프·기계 핸드오프(S1)에서도 붙이지 않는다 — 읽을 사람이 없다.
